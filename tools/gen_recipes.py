...
# Also emit each section's table as a reusable snippet fragment, so content pages can embed the exact
# same recipes via  --8<-- "recipes/<slug>.md"  (single source of truth; the slug matches the section's
# anchor on the Recipe Reference page). Fragments live outside docs/ so they aren't built as pages.
INCLUDES = os.path.join(REPO, "includes/recipes")
os.makedirs(INCLUDES, exist_ok=True)

def slugify(t):
    s = re.sub(r"[^\w\s-]", "", t).strip().lower()
    return re.sub(r"[-\s]+", "-", s)

frag_index = []
for title, _intro, table in S:
    slug = slugify(title)
    with open(os.path.join(INCLUDES, slug + ".md"), "w", encoding="utf-8") as f:
        f.write(table.rstrip() + "\n")
    frag_index.append((slug, title))
# a small index so authors/maintainers can see what's available
with open(os.path.join(INCLUDES, "README.md"), "w", encoding="utf-8") as f:
    f.write("# Recipe snippet fragments\n\nGenerated by `tools/gen_recipes.py`. Embed on a content "
            "page with, e.g.:\n\n```\n??? example \"Recipes: ...\"\n\n    --8<-- \"recipes/<slug>.md\"\n"
            "```\n\nAvailable fragments (slug — section, also the anchor on `reference/recipes.md`):\n\n")
    for slug, title in frag_index:
        f.write(f"- `{slug}` — {title}\n")

n_icons = sum(1 for v in _icons.values() if v)
print(f"Wrote {OUT}")
print(f"Sections: {len(S)}   Fragments: {len(frag_index)}   Icons: {n_icons}   "
      f"(unresolved: {sum(1 for v in _icons.values() if not v)})")
